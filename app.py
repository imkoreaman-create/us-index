...
def force_editor_rebuild():
    """표의 인덱스 캐시를 삭제하여 체크박스가 꼬이지 않게 만듭니다."""
...
with col_top2:
    if st.button("🔄 전체 새로고침", use_container_width=True):
        ...
        st.rerun()
with col_top3:
    st.info(f"마지막 갱신: {st.session_state.last_update} (새로고침해도 순서가 유지됩니다)")

# [종목 추가/수정 패널 - 에러 완벽 해결]
with st.expander("➕ 새로운 종목 추가 및 수정", expanded=False):
    ...
    c1, c2 = st.columns(2)
    # value 대신 key 속성만 사용하여 Streamlit 에러 원천 차단
    ...
    if bc1.button("➕ 종목 추가", use_container_width=True):
        if new_name and new_ticker:
            ...
            st.session_state.form_name = ""
            st.session_state.form_ticker = ""
            st.session_state.db_choice = "직접 입력"
            force_editor_rebuild()
            st.rerun()
...
# 가격/등락률을 문자열이 아닌 '숫자(Float)'로 넣어 Streamlit이 자동으로 우측 정렬하게 만듦
df_list = []
...
# 숫자형 컬럼 포맷팅 적용 (자동 우측 정렬)
col_config = {
    "✅선택": st.column_config.CheckboxColumn("✅선택"),
    "항목": st.column_config.TextColumn("항목"),
    "추세": st.column_config.TextColumn("추세"),
    "현재가": st.column_config.NumberColumn("현재가"), # 소수점/콤마 자동 최적화
...
if st.button("▶ 실시간 포트폴리오 AI 분석 실행", use_container_width=True, type="primary"):
    with st.spinner('실제 포트폴리오 데이터를 스캐닝하여 분석 중입니다...'):
        ...
        # 2) 실제 보유 종목 중 상승률/하락률 Top 종목 추출 (매크로 지수 제외)
        ...
        if real_stocks:
            ...
            g_name = top_gainer[0]
            g_price = f"{top_gainer[1]['raw_price']:,.0f}" if top_gainer[1]['raw_price'] > 1000 else f"{top_gainer[1]['raw_price']:,.2f}"
            g_chg = top_gainer[1]['raw_change']
            
            l_name = top_loser[0]
            l_price = f"{top_loser[1]['raw_price']:,.0f}" if top_loser[1]['raw_price'] > 1000 else f"{top_loser[1]['raw_price']:,.2f}"
            l_chg = top_loser[1]['raw_change']
            
            insight_text = f"""
            <li><b>{g_name} (핵심 주도주):</b> 현재가 <b>{g_price}원</b> (<span style='color:#ff4d4d;'>{g_chg:+.2f}%</span>). 알고리즘 스캐닝 결과 내 포트폴리오에서 가장 강력한 수급 모멘텀을 형성 중입니다. <b>{algo_sel}</b> 모델 기준 단기 추가 상승 여력이 높습니다.</li>
            <li><b>{l_name} (리스크 관리):</b> 현재가 <b>{l_price}원</b> (<span style='color:#4d94ff;'>{l_chg:+.2f}%</span>). 현재 단기 지지선 테스트 구간에 진입했습니다. 기술적 반등이 없을 경우 비중 축소를 권장합니다.</li>
            """
        else:
